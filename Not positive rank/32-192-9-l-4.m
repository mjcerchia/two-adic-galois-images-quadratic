...
Let C be the modular curve with lmfdb label 32.192.9.l.4.
...
C := Curve(P,[z*s - r^2, z^2 + z*s + v^2, x*s - t*r, x*z + x*s - u*v, t*v + u*r, w*s - t^\ 2, x^2 + w*s + u^2, x*t - w*r, x*u + w*v, x*v + z*u, x*r - z*t, x^2 - z*w, x^2 + z*w + v*r,\
 x*t + w*r + v*s, x*s - y^2 + u*v, x*u + z*r - w*v + r*s, 2*w*t - u*s, 2*x*w - u*r, x*r + 2\
*w*u + t*s, 2*w^2 - t*u, z*s + 2*t*u + s^2]);
