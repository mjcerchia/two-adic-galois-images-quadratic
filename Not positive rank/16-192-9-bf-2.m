...
Let C be the modular curve with lmfdb label 16.192.9.bf.2.
...
C := Curve(P,[x*t + w*r, t*u - t*r + r*s, t^2 - t*r + u*r, t^2 - t*v - v*r + r*s, t*s - u^2 + u*r - r*s, x*t - x*s + w*u, x*t - x*u + w*t, t*v + t*r - t*s - u*v - v*r, 2*x*w - t*v + v*s, t^2 - t*v - t*s - u*v + u*s + v*s, x*v - x*s + w*t - w*v, x*u - x*r - z*s, x*v - x*r - z*v + w*t, x*t - x*r - z*u, x*r + z*t, x^2 - z*w, x^2 + z*w + u*v, 2*w^2 + t*v - v*r + r*s - s^2, 2*x*z + t*v - v*r, x*v - x*s - 2*y^2 + z*s, 2*z^2 + t*v - v*r + r^2 - r*s]);
