...
Let C be the modular curve with lmfdb label 32.192.9.br.2.
...
C := Curve(P,[x^2 + u*v, x^2 - y*u + t*s, z*s + w*u, z*r - z*s + t*u, x^2 + \
y*u + w*r, w*r - w*s - t*s, 2*y*r - y*s + v*s, y*r - y*s + z*s - w*u + v*r, y*\
z + z*v + w*t, y*u - 2*z*u - t*r, y*w + y*t - 2*z*w - t*v, 2*z^2 + z*v - t^2, \
y*w + 2*y*t + w*v, y*z - z*v - w^2 - w*t, 2*z^2 + w^2 + w*t + t^2 + u^2, y^2 -\
 y*z + z*v - w^2 + w*t + v^2, y*w - y*t + 2*z*w - w*v - t*v - u*s, y^2 - 2*z^2\
 + w^2 - w*t - t^2 + u^2 - r^2 + r*s, y^2 + y*z - z*v + w^2 - w*t - 2*u^2 + r^\
2 - r*s + s^2, y^2 + y*v + 2*z*v + w^2 - 2*w*t - r*s + s^2, y*t + 4*z*t - u*r \
+ u*s]);
