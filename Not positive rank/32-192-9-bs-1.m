...
Let C be the modular curve with lmfdb label 32.192.9.bs.1.
...
C := Curve(P,[x*s - u*r, x*s + t*v, y*s + u*v, x*v + y*r, x*u - y*t, w*v + w\
*r - t*r, w*t - w*u - t^2, 2*v^2 + 2*v*r - s^2, x^2 - 2*u^2 - v^2 - v*r, 2*x^2\
 + w*u, x*r + z^2 + t*s, x*r - z^2 + w*s - t*s, x*v - y*r - w*s + t*s, x*w - x\
*t - y*w, 2*x*y + w*u - t*u, 2*y^2 + w*u - t*u - u^2, 2*y*v - w*s + t*s + u*s,\
 x*w + 2*x*u + 2*y*t + r*s, y*w + 4*y*u - v*s, 2*x*y + 2*y^2 - w^2 - w*u + t^2\
 + 2*t*u - u^2 - 2*v*r, 2*x^2 - w^2 - w*t - 2*w*u + t^2 + 2*r^2]);
