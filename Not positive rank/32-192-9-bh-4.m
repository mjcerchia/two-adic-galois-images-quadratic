...
Let C be the modular curve with lmfdb label 32.192.9.bh.4.
...
C := Curve(P,[z^2 - v*r, x*z - u*v, x*z - t*r, x^2 - t*u, x^2 - z*w, x*u - w\
*r, x*t - w*v, x*r - z*u, x*v - z*t, x*s + y^2, x*s - y^2 + t*v - u*r, 2*z*s +\
 v^2 - r^2, 2*w*s + t^2 - u^2, x^2 + z*w + z*s + 2*t*u - r^2, z^2 - 2*t^2 - 2*\
u^2 + s^2, 2*x*u - z*v + 2*w*r - r*s, 2*x*t - z*r + 2*w*v + v*s, x*v - 4*w*u +\
 u*s, 4*x*w + x*s - u*r, 4*w^2 + w*s - u^2, x*r - 4*w*t - t*s]);
